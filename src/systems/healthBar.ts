...
const HEALTH_BAR_FILL = "HealthBarFill";
...
export function attachHealthBar(holder: Object3D): void {
  ...
  const background = new Mesh(
    ...
    new MeshBasicMaterial({ color: 0x263845, depthWrite: false }),
  );
  ...
  const fill = new Mesh(
    ...
    new MeshBasicMaterial({ color: 0x22c55e, depthWrite: false }),
  );
  fill.name = HEALTH_BAR_FILL;
...
export function updateHealthBar(entity: Entity): void {
  ...
  const fill = bar?.getObjectByName(HEALTH_BAR_FILL) as Mesh | undefined;
  if (!bar || !fill) return;
  ...
  material.color.setHex(
    ratio > 0.6 ? 0x22c55e : ratio > 0.3 ? 0xfacc15 : 0xef4444,
  );
}
